...
class BinancePNLTracker:
    def __init__(self):
        ...
        self.cloud_functions_url = "https://YOUR_REGION-YOUR_PROJECT.cloudfunctions.net"
...
/help - Bu yardım mesajını gösterir"""
    ...
    def sync_to_cloud(self):
        """PNL verilerini Cloud'a senkronize et"""
        if not self.cloud_enabled:
            return
            
        try:
            data = {
                'total_pnl': float(self.current_pnl_var.get().split(': ')[1].split()[0]),
                'high_pnl': self.high_pnl,
                'low_pnl': self.low_pnl,
                'positions': [p for p in self.client.futures_position_information() 
                            if float(p['positionAmt']) != 0]
            }
            
            response = requests.post(
                f"{self.cloud_functions_url}/track_pnl",
                json=data
            )
            
            if response.status_code != 200:
                print(f"Cloud sync error: {response.text}")
                
        except Exception as e:
            print(f"Cloud sync failed: {e}")
    
    def create_cloud_alarm(self, symbol, target_pnl, alarm_type):
        """Cloud üzerinde yeni alarm oluştur"""
        try:
            data = {
                'symbol': symbol,
                'target_pnl': target_pnl,
            ...
            response = requests.post(
                f"{self.cloud_functions_url}/manage_alarms",
                json=data
            )
            
            if response.status_code == 200:
                print("Cloud alarm created successfully")
            else:
                print(f"Cloud alarm creation failed: {response.text}")
                
        except Exception as e:
            print(f"Cloud alarm creation error: {e}")
